# Histórias de Usuário - Classi-O

## 1. Eu como vendedor desejo realizar um cadastro no sistema.
### Tipo de usuário
Vendedor
### Ação
Realizar seu cadastro no sistema
### Motivo da Ação
Para realizar um anúncio no sistema, preciso fazer o meu cadastro pessoal.

## 2. Eu como vendedor desejo editar o cadastro no sistema.
### Tipo de usuário
Vendedor
### Ação
Editar seu cadastro no sistema
### Motivo 
Para realizar um anúncio no sistema, preciso do meu cadastro pessoal atualizado.

## 3. Eu como vendedor desejo fazer o login no sistema usando um email e uma senha
### Tipo do usuário
Vendedor
### Ação
Realizar meu login no sistema
### Motivo da Ação
Para ter maior segurança, o sistema é acessado apenas com um Email e senha.

## 4. Eu como vendedor desejo excluir meu cadastro pessoal
### Tipo do Usuário
Vendedor
### Ação
Excluir meu Usuário
### Motivo da Ação
Desativar sua conta, caso o vendedor não tenha mais interesse de usar o sistema.

## 5. Eu como vendedor desejo realizar um logoff do sistema.
### Tipo do Usuário
Vendedor
### Ação
Realizar logoff
### Motivo da Ação
Para manter a conta fora de atividade, preciso realizar um logoff no sistema.

## 6. Eu como vendedor desejo recuperar a senha de usuário do sistema.
### Tipo do Usuário
Vendedor
### Ação
Recupar senha
### Motivo da Ação
Para manter maior segurança, preciso recuperar minha seja caso esqueça.

## 7. Eu como vendedor desejo criar meu próprio anúncio no sistema.
### Tipo do Usuário
Vendedor
### Ação
Criar anúncio
### Motivo da Ação
Para que o vendedor possa publicar um anúncio, preciso criar um anúncio próprio.

## 8. Eu como vendedor desejo excluir meu próprio anúncio no sistema.
### Tipo do Usuário
Vendedor
### Ação
Excluir anúncio
### Motivo da Ação
Para que o vendedor possa excluir uma publicação, preciso excluir o anúncio publicado.

## 9. Eu como vendedor desejo editar meu próprio anúncio no sistema.
### Tipo do Usuário
Vendedor
### Ação
Editar anúncio
### Motivo da Ação
Para que o vendedor possa editar uma publicação, preciso editar o anúncio que está publicado.

## 10. Eu como vendedor desejo visualizar meu próprio anúncio no sistema.
### Tipo do Usuário
Vendedor
### Ação
Visualizar anúncio
### Motivo da Ação
Para que veja os anúncios publicados, preciso visualizar cada anúncio que foi publicado pelo vendedor.

## 11. Eu como usuário desejo pesquisar por anúncios no sistema.
### Tipo do Usuário
Usuário
### Ação
Pesquisar anúncio
### Motivo da Ação
Para que veja os anúncios publicados, preciso pesquisar por anúncios publicados.

## 12. Eu como usuário desejo escolher o filtro de pesquisa no sistema
### Tipo do Usuário
Usuário
### Ação
Filtrar anúncio
### Motivo da Ação
Para que veja os anúncios publicados, preciso filtrar os anúncios de acordo com a preferência.

## 13. Eu como usuário desejo expandir os anúncios no sistema.
### Tipo do Usuário
Usuário
### Ação
Expandir anúncio
### Motivo da Ação
Para que veja os anúncios publicados, preciso expandir os anúncios pesquisados.

## 14. Eu como usuário desejo visualizar todos os anúncios no sistema.
### Tipo do Usuário
Usuário
### Ação
Visualizar anúncio
### Motivo da Ação
Para que veja os anúncios publicados, preciso visualizar cada anúncio que foi publicado pelo vendedor.

## 15. Eu como usuário desejo enviar um e-mail ao vendedor.
### Tipo do Usuário
Usuário
### Ação
Enviar e-mail
### Motivo da Ação
Para que tenha uma comunicação entre vendedor e usuário, preciso que o usuário consiga enviar um e-mail ao vendedor referente.


